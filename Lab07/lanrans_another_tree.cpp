...
bool comp_one(Edge *a, Edge *b) { return a->hasWeight; }

bool comp_zero(Edge *a, Edge *b) { return b->hasWeight; }
...
int main() {
  ...
  while (t--) {
    ...
    Edge **pq_one = new Edge *[m], **pq_zero = new Edge *[m];
    for (int i = 1; i <= n; i++) {
    ...
    for (int i = 0; i < m; i++) {
      cin >> u >> v >> w;
      pq_one[i] = new Edge{grp_one[u], grp_one[v], w};
      pq_zero[i] = new Edge{grp_zero[u], grp_zero[v], w};
    }
    sort(pq_one, pq_one + m, comp_one);
    sort(pq_zero, pq_zero + m, comp_zero);
    max_cnt = min_cnt = 0;
    for (int i = 0; i < m; i++) {
      temp_edge = pq_one[i];
