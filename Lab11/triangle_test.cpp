...
void FFT(complex<double> *fft_coe, int len, bool inv) {
  ...
  while (wid <= len) {
    ...
    for (int i = 0; i < len; i += wid) {
      for (int k = 0; k < mid; k++) {
        if (inv) {
          temp = conj_inv[len / wid * i] * fft_coe[i + k + mid];
        } else {
          temp = omega[len / wid * i] * fft_coe[i + k + mid];
        }
...
int main() {
  ...
  for (int i = 1; i <= max_len; i++) {
    num_of_sum[i] = (ll)fft_coe[i].real();
  }
  for (int i = 0; i < n; i++) {
    num_of_sum[2 * l[i]]--;
  }
  ...
  for (int i = 1; i <= max_len; i++) {
    num_of_sum[i] += num_of_sum[i - 1];
  }
  ll rst = 0;
  for (int i = 0; i < n; i++) {
    rst += num_of_sum[max_len] - num_of_sum[l[i]];
    rst -= (n - i - 1) * (ll)i;
    rst -= (n - 1);
    rst -= (n - i - 1) * (n - i - 2) / (ll)2;
  }
  cout << rst;
  return 0;
}